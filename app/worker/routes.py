...
async def get_jobs_near_location(request: Request):
    ...
    try:
        ...
        for req in citizen_requests:
            ...
            # Check if location is a string or dict
            if isinstance(req["location"], str):
                location_address = req["location"]
            elif isinstance(req["location"], dict):
                ...
                if not location_address:
                    ...
                    location_address = f"{lat:.4f}, {lng:.4f}"
            else:
                location_address = "Unknown Location"
            
            # Extract coordinates safely
            if isinstance(req["location"], dict):
                req_lat = req["location"].get("latitude", latitude)
                req_lng = req["location"].get("longitude", longitude)
            else:
                req_lat = latitude
                req_lng = longitude
            ...
            all_jobs.append({
                ...
                "duration": 15,
                "distance": calculate_distance(latitude, longitude, req_lat, req_lng),
...
async def accept_job(request: Request):
    ...
    try:
        data = await request.json()
        job_id = data.get("job_id")
        job_type = data.get("job_type", "request")
        worker_id = request.cookies.get("user_session", "demo_worker")
